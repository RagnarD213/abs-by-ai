...
| the ledger | **Google.** Ad names carry the video title and id (`AUTO test · <video title> · yt:<id> · tier2 · 2026-09-08`; titles added 2026-09-08 at Dan's request so he can review ads by name — ads named before that are renamed once by a `renameAd` command), labels carry the state (`AUTO`, `AUTO:TEST`, `AUTO:CHAMPION`, `AUTO:RETIRED`, `AUTO:RETIRED-DAY1`). "Has this video been tested here?" is answered by the snapshot, so a re-run can never double-create |
