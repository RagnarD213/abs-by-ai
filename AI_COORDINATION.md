...
- **A3 — female body-fat anchor retune (PRIMARY-goal, highest regression risk).** Lower the female anchors/floor one step + strengthen the female prompt so the female TARGET is more dramatic. The handoff requires comparing against current prod on REAL female photos before shipping. **Open question for Dan:** now that A2 removed the verifier false-negatives (the proof-photo female dramatic result already passes cleanly), how aggressive should A3 be — or does he want to first eyeball a few of his own real female photos on prod to decide if A3 is even needed?
