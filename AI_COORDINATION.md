...
- **Phase 4 IN PROGRESS.** Shared injector `profileContextBlock(profile)` renders a compact labeled background block (`''` when empty → graceful). **4a: Daily Coach Brief COMPLETE, commit `37972ab`** — profile prepended to the brief prompt + folded into the cache fingerprint. Prod boots healthy (route intact).
  - **Remaining Phase 4 features + design note:** Trainer + Nutritionist already collect full intakes that duplicate the profile — their best win is CLIENT-SIDE pre-fill (skip answered questions), a larger separate effort, not server prompt injection. Macro Tracker (analyze-meal) gets little from profile (it itemizes a photo). Sleep Coach = modest server context. Supplement Audit = server context BUT has tuned evals (regression risk — add background only, re-run evals). **Live-verifying these member-gated AI features needs a comp/test member account (local Anthropic key is invalid) — pending Dan's steer.**
- **Phase 5 (write-backs + full new-user run) NOT STARTED.**
