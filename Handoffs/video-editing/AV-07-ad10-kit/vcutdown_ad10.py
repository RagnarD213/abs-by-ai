#!/usr/bin/env python3
"""AV-07 -- Ad 10's <=0:59 9:16 cutdown, selected from the finished kit vertical.
...
Selecting from the finished full vertical carries its take choice, grade, graphics,
framing, and measured labels through unchanged. The three ranges tell one complete story:
hook/problem/AI solution; generated-image demo and payoff; final app beat and CTA. Their
edges land on silence and picture boundaries from Muhammad's approved cut.
...
# Ad 10's required doctrine: hook, problem, AI demo, payoff, CTA. The first range is
# deliberately continuous so the before/after premise and the answer remain coherent.
RANGES = [
 (0.0, 901 / FPS),
 (2704 / FPS, 3242 / FPS),
 (5170 / FPS, BT.DUR),
...
def main():
    ...
    lab, ins = [], []
    json.dump([], open('cut/labels.json', 'w'))
    ...
    sh([FF, '-nostdin', '-v', 'error', '-y', '-i', 'cut/picture.mp4', '-i', 'cut/captions.mov',
        ...
        '-c:a', 'aac', '-aac_coder', 'fast', '-b:a', '320k', '-ar', '48000', '-ac', '2',
        '-movflags', '+faststart', 'ad10_vertical_59s.mp4'])
    got = nframes('ad10_vertical_59s.mp4')
    assert got == total, f'cutdown is {got} frames, plan is {total}'
    print(f'ad10_vertical_59s.mp4 done -- {got} frames, {got/FPS:.2f}s')
