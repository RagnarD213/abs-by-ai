...
## Source
* **Roll:** `/Volumes/Extreme/abs by ai 8:28 shoot | jeff | dan | ads, dedicated shorts, b roll, scripted long form content/main camera/` **C1671 ≈0:00–1:00**. Times are approximate, from a Whisper pass. Full transcript: `/Volumes/Extreme/_edit_work/_transcripts-828-full/`. The range holds the slate
  (*"the video I'm about to film is…"*), every take and crew chatter.
* **Script:** Shoot 5 scripts doc, Drive `1yZjcG5pkbw0kPsfTvc7OOr2bX6v0bVYMqquUiRENQ4k` (local plain-text copy: `Media/codex-video-trial/05-recipes/candidates/shoot5-notes.txt`), lines **857–867** (section 7, short-form workout scripts ("Dedicated shorts workout content. Vertical 9:16."); b-roll list in section 8).
...
* B-roll: 8/28 C1673 (kettlebell deadlift), done right plus the mistakes: weight in front, toes out, rounded back, hip thrust at the top. Label each mistake on screen (red ✕), and the right way (green ✓).

## Deliver
`Short-form video content/ds-18_how-to-kettlebell-deadlift.mp4` + stamps (no covers: separate Codex task). **Don't upload or queue.** Organic shorts go out through Blotato, uploaded Private (`/video-setup` step, separate). Send Dan the review copy. Update `00-MASTER.md`.
