# RA-09 — "Do You Think AI Is Changing The World?": short-form ad, first cut from raw footage
...
## Source
* **Roll:** `/Volumes/Extreme/abs by ai 8:28 shoot | jeff | dan | ads, dedicated shorts, b roll, scripted long form content/main camera/` **C1666 whole roll (1:25)**. Times are approximate, taken from a Whisper pass. Full transcript: `/Volumes/Extreme/_edit_work/_transcripts-828-full/` (one `.txt` per roll).
  The range holds every take of this script: slate (*"the video I'm about to film is…"*), retakes and crew chatter.
* **Script:** Shoot 5 scripts doc, Drive `1yZjcG5pkbw0kPsfTvc7OOr2bX6v0bVYMqquUiRENQ4k` (local plain-text copy: `Media/codex-video-trial/05-recipes/candidates/shoot5-notes.txt`), lines **355–364** (section 3, short-form ad scripts, *"Paid ads, not organic content. Every one ends on a 'tap the button below' call to action."*).
...
## This ad's specifics
* A near-twin of RA-10 (same body, different hook). Build them from shared assets so Dan can A/B the hooks.

## Deliver
New folder `Claude Ad Videos/do you think ai is changing the world - RA-09/` (or `Codex Ad Videos/…`):
`… | claude | 9x16 | RA-09.mp4`, `… | claude | 16x9 | RA-09.mp4` + review copies + stamps. Send Dan the 9:16 review copy. Update `00-MASTER.md`.

## Starter prompts
**Claude (Fable 5.1, high):**
> Read `Handoffs/video-editing/00-RULES.md`, then execute `Handoffs/video-editing/RA-09-do-you-think-ai-is-changing-the-world.md`: cut the "Do You Think AI Is Changing The World?" short-form ad from its 8/28 raw takes with /ad-edit. Best take of every line, 9:16 master ≤0:59 plus the 16:9 from the same edit, lav picked per file, the 8/28 S-Log3 conversion, same-person before/after, claim lines flagged to me. Every gate, independent audit, deliver, send me the review copy, update the master list.

**Codex (GPT-6 Astra, high):**
> Read `Handoffs/video-editing/00-RULES.md` (Codex column + environment table), then execute `Handoffs/video-editing/RA-09-do-you-think-ai-is-changing-the-world.md` with `$abs-edit-ad`: 9:16 master ≤0:59 plus 16:9 from the same edit. Deliver, send Dan the review copy, update `00-MASTER.md`.
