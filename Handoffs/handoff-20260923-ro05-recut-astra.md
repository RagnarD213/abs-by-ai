# RO-05 recut from scratch in Codex (GPT-6 Astra): "How I Make My Daily Salad"

**Written 2026-09-23 by Claude (Opus 5.5) after Dan rejected Claude's cut.** Executor: Codex session, **GPT-6 Astra, effort high**. Job: RO-05 on
...
- Skill: `$abs-edit-organic` (`Media/codex-video-trial/skills/abs-edit-organic/SKILL.md`) and its references; start from the approved
  ...
  zsh word-splitting, `pgrep` self-matching, names containing ` | `).
- Work dir: `/Volumes/Extreme/_edit_work/ro05-astra/` (never write into `ro05/`). Two-build machine cap applies.
- Queue: `python3 scripts/edit-queue/queue.py set RO-05 in_progress --by "Codex (Astra recut)"` and commit `jobs.json` + `00-MASTER.md`.
  Claude is running the same job in parallel on purpose (Dan's bake-off); do not stop it. Git: commit docs/scripts only, never media;
  follow the branch rule in `Handoffs/codex-video-trial/00-start-here.md`.
- Deliver to `Codex Content Videos/how i make my daily salad/`: master `How I Make My Daily Salad | codex | 16x9 | RO-05.mp4`, `.srt`,
