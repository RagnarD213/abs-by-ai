...
const SALT = findBalancedSalt(CASES.map((c) => c.id), cellsFor);
...
<div class="wrap">
  <h1>Female model test - blind</h1>
  <p class="lede">Right now every female transformation effectively runs on <strong>one</strong> model, because the second model in the pair refuses about three out of four female photos. This test asks whether a replacement is good enough to take that slot. Each row below shows the same woman transformed by <strong>two different models</strong>. Names are hidden and the letters are shuffled per row, so A in one row is not the same model as A in the next.</p>
  <ol class="steps">
  ...
  <p class="warn">Tap any image to enlarge. Answers save in this browser as you go. Note: rows 1-4 are the same starting photo - the difference is which starting body type a user would have declared, which changes what the app promises her.</p>
  ${caseBlocks.map((b) => `
...
<script>
const KEY = 'absbyai-bakeoff-round2-female';
