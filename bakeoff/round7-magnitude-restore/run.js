// Round-7 batch: generate the NEW-prompt arm only (old arm = round-5 images,
// which were generated under the current production prompt and are already
// Dan-labelled). Single-shot per cell + production's Gemini safety retry, no
// deviceId, successful cells never re-spend. SET=1 gemini only, SET=2 flux only.
const fs = require('fs');
...
(async () => {
  ...
  for (const c of CASES) {
    for (const arm of ARMS) {
      ...
      // Round-6 bug, fixed here: this skipped ANY cell whose .json existed,
      // including failed {ok:false} records. After the Gemini outage a re-run
      // reported "cached" for all six failures and generated nothing. A record
      // only counts as cached if it succeeded AND its image is on disk.
      if (fs.existsSync(metaPath)) {
        let prev = null;
        try { prev = JSON.parse(fs.readFileSync(metaPath, 'utf8')); } catch (_) { prev = null; }
        if (prev && prev.ok && fs.existsSync(imgPath)) { console.log('cached', stem); ok++; continue; }
        console.log(`retry  ${stem} (previous record ok=${prev ? prev.ok : 'unparseable'})`);
