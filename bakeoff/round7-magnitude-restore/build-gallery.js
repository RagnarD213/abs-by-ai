// Blind A/B: CURRENT-PRODUCTION prompt (old) vs MAGNITUDE-RESTORED prompt (new),
// model held constant per row. Old arm = Dan's labelled round-5 images (which
// were generated under today's production prompt); new arm = out/.
//
...
const html = `<title>Abs By AI - male muscle-magnitude test (blind A/B)</title>
<style>
  :root {
...
<div class="wrap">
  <h1>Did turning the muscle back up fix the male results?</h1>
  <p class="lede">You asked why the male results got worse than they were at the beginning. They did, and it was our own instructions: back in July we cut the amount of added muscle roughly in half and swapped words like "visibly bigger" for "slightly fuller". I have <strong>put the original muscle amounts back</strong> - while keeping the two fixes that worked (no added tan, and never bodybuilder-sized). Every row below is the <strong>same man, same model</strong> - the only difference is the old instructions vs the restored ones. Which is which is hidden and shuffled per row.</p>
  <ol class="steps">
  ...
  ${(() => {
    ...
    for (const b of blocks) {
      if (b.set !== lastSet) {
        lastSet = b.set;
        out += b.set === 1
          ? `<div class="sethdr"><h2>Set 1 - the main event (Gemini leg)</h2><p>This is the leg that went backwards: last round you rejected BOTH images in 6 of 6 male rows here, every one tagged "not enough change". These rows decide whether the restore ships.</p></div>`
          : `<div class="sethdr"><h2>Set 2 - the FLUX leg (control)</h2><p>FLUX already over-changed men (8 of 12 last round). The restored instructions reach this leg too, so the check here is that they did not push it further over - "too muscular" / "too much change" getting clearly worse on this side counts against shipping, because production sends the change to both legs or neither.</p></div>`;
      }
      out += `
...
<script>
const KEY = 'absbyai-round7-magnitude-restore';
