...
for (const seg of (ONLY.length ? SEGMENTS.filter((s) => ONLY.includes(s.id)) : SEGMENTS)) {
  ...
  // and that the measured head clearance the geometry was chosen for still holds
  check(T.headY - L.dropTop >= 60,
    `head clears the title band by only ${T.headY - L.dropTop}px`);
