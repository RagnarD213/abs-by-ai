...
CUTS = [s['cut'] for s in SP] + [1827.751]

# ALL 49 beats of the Zepbound master are measured up front, so the shortlist can be cut
# without a second measurement pass. Keyed by beat index only (mkcrops/mkplan look up by beat).
WANT = {'ALL': list(range(len(CUTS) - 1))}
