...
for (const seg of (ONLY.length ? SEGMENTS.filter((s) => ONLY.includes(s.id)) : SEGMENTS)) {
  ...
  // ab-wheel's stage-vs-caption-band check does not apply; what does is that the source
  // window really is 2:3 (or the picture is stretched) and that it fits inside the frame.
  const T = L.talk;
  const ph = 1920 - L.dropTop;
  check(Math.abs(T.cropW / T.cropH - 1080 / ph) < 0.002,
    `talk window ${T.cropW}x${T.cropH} is not the picture's ${1080}x${ph} aspect - it would stretch`);
  check(T.cropTop + T.cropH <= 1080, `crop runs past the bottom of the source frame`);
  // ⚠ ZEPBOUND: his head is at SOURCE ROW 0 on every beat, so headY == dropTop by construction
  // and the old ">=60px above dropTop" margin is unattainable and meaningless here. What the
  // standing rule actually needs is black between the title's INK and the top of the picture.
  // Measure it off the rendered title PNG: lowest solid glyph pixel vs dropTop.
  check(T.headY >= L.dropTop, `picture would start above dropTop (headY ${T.headY} < ${L.dropTop})`);
  {
    const inkBottom = parseInt(run('python3', ['-c',
      `from PIL import Image; import numpy as np; a=np.array(Image.open('assets/title-${seg.id}.png').split()[-1]); ys=np.nonzero(a>200)[0]; print(ys.max())`]).trim(), 10);
    console.log(`   title ink ends y=${inkBottom}, picture starts y=${L.dropTop} (${L.dropTop - inkBottom}px of black)`);
    check(L.dropTop - inkBottom >= 30, `title ink (y${inkBottom}) is within 30px of the picture top (y${L.dropTop})`);
  }

  // ⚠ CHECK: the caption-sync gate must have SEEN THIS EXACT FILE. Added 2026-08-28 after
  // this batch passed every other check while shipping captions up to 650 ms late and
  // clipping the first word off two shorts - the analysis WAV and the `-ss` cuts were on
  // different timelines and nothing here compared audio against captions. Run
  // `python3 syncgate.py` after rendering.
  const stampPath = path.join(__dirname, 'build', 'syncgate.json');
  const stamps = fs.existsSync(stampPath) ? JSON.parse(fs.readFileSync(stampPath, 'utf8')) : {};
  const st = stamps[seg.id];
  if (!check(!!st, 'caption-sync gate has not run on this short - run syncgate.py')) {
    // fall through, nothing else to say
  } else {
    check(Math.abs(st.mtime - fs.statSync(f).mtimeMs / 1000) < 1.5,
      `caption-sync gate ran on a DIFFERENT build of this short - re-run syncgate.py`);
    check(st.ok === true,
      `caption-sync gate failed: median ${st.median_ms} ms` + (st.clipped ? ', first word clipped' : ''));
  }
