...
// ⚠ ZEPBOUND BATCH: THE SOURCE'S AAC HAS ~13 ms OF pts OVERLAP AT EVERY ONE OF ITS 48 JOINS
// (85,706 frames x 1024 = 622 ms more samples than the container declares). A plain `-ss/-t`
// pull that spans a join comes out 14.5 ms LONG and every word after the join lands late
// against the picture. Measured: with this hard-comp resample in front, the pull is exactly
// its requested length and post-join audio sits at 0.00 ms (corr 1.000). Same fix as the
// analysis wav (see work/TIMELINE_TRAP.md).
const TIMELINE_FIX = 'aresample=async=1:min_hard_comp=0.005:first_pts=0';
const VOICE = TIMELINE_FIX + ',' + fs.readFileSync(path.join(__dirname, 'work', 'voicechain.txt'), 'utf8').trim();
