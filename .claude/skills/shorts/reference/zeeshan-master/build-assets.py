...
def chip(shot, label, w, h, size=30):
    ...
    x = pad
    for c in label:
        d.text((x, h // 2 - int(size * 0.6)), c, font=f, fill=OLIVE + (255,))
        x += f.getlength(c) + 5
...
for shot, label in json.load(open(os.path.join(HERE, 'shots', 'labels.json'))).items():
    print(chip(shot, label, L['card']['w'], 84, size=44))
OV = json.load(open(os.path.join(HERE, 'overlays.json')))
