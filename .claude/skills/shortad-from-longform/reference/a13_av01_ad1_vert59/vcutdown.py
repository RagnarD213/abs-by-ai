#!/usr/bin/env python3
"""AV-01 -- the <=0:59 9:16 VERTICAL cutdown: intervals SELECTED out of the finished square master.

  python3 vcutdown.py            # plan only, prints the transcript to read as prose
  python3 vcutdown.py --build    # cut the picture + his mix, rebuild the captions, mux
...
# ⚠ THE VERTICAL MASTER IS 6,977 FRAMES, ONE MORE THAN MUHAMMAD'S 6,976 (and one more than the
# square). Attempt 3 predates the frame-count assert, so its final mux took its length from the
# longest input. PROVEN, not assumed: proof/align_frames.py correlates the two masters' frame-to-
# frame difference profiles in 200-frame windows across the whole film -- lag 0 in every window
# (global r=0.975 at lag 0 against 0.48 at lag +-1). The extra frame is at the TAIL, so vertical
# frame N is square frame N for N <= 6975 and the square's proven frame indices transfer as they
# are. Only the final range's clamp changes.
MASTER_FRAMES = 6977
...
def main():
    ...
    # ---- the labels this cutdown ADDS ---------------------------------------
    # ⚠ THE APPROVED VERTICAL PREDATES DAN'S 2026-09-11 LABEL RULE, so three pictures inside the
    # selection carry no chip: his two "this is where I'm at today" photos (real after pictures)
    # and the app's "Download Your Future Self" screen (a generated result -- lesson A6.13). The
    # square added all three. They go on HERE, in the mux, rather than by re-rendering the beats:
    # the cutdown is a selection out of the approved master, so re-rendering would change pixels
    # Dan has approved, and assert_range_lands() would no longer be able to prove the selection
    # against that master. Placements are MEASURED on these very frames by vlabelplace.py (person
    # mask, 16 px clearance, above the head first) -- never a fixed y.
    import chips as CH
    ...
    for media, c in place.items():
        w = None
        for p in P:
            a0, a1 = max(c['beat'][0], p['src0']), min(c['beat'][1], p['src1'])
            if a1 - a0 > 0.02:
                w = (p['dst0'] + (a0 - p['src0']), p['dst0'] + (a1 - p['src0'])); break
        if w is None:
            print(f'  label {media}: not in the cutdown, skipped'); continue
        png = f'cut/chip_{media}.png'
        CH.chip_at(CH.REAL_LABEL if c['kind'] == 'real' else CH.AI_LABEL,
                   c['x'], c['y'], c['lines'], c['size']).save(png)
        lab.append((media, png, w, c))
        print(f"  label {media:14s} {c['kind']:4s} {c['w']}x{c['h']} at {c['x']},{c['y']}  "
              f"cutdown {w[0]:.3f}-{w[1]:.3f}s")
    json.dump([dict(media=m, png=p_, t0=round(w[0], 4), t1=round(w[1], 4),
                    box=[c['x'], c['y'], c['w'], c['h']], kind=c['kind'])
               for m, p_, w, c in lab], open('cut/labels.json', 'w'), indent=1)

    # ---- mux: his cut mix, AAC 320k, no filter of any kind ------------------
    chain, cur = [], '0:v'
    for i, (media, png, w, c) in enumerate(lab):
        ins += ['-loop', '1', '-framerate', '30000/1001', '-i', png]
        nxt = f'l{i}'
        # ⚠ HALF-OPEN, ON HALF-FRAME BOUNDARIES. `between(t,a,b)` is inclusive at both ends, and
        # today_towel ends at exactly the frame today_trees begins -- so an inclusive pair printed
        # BOTH chips on the join frame. Switching midway between frame n-1 and frame n makes each
        # chip cover exactly its beat's own frames.
        a_ = (round(w[0]*FPS) - 0.5)/FPS
        b_ = (round(w[1]*FPS) - 0.5)/FPS
        chain.append(f"[{cur}][{3+i}:v]overlay=0:0:enable='between(t,{a_:.5f},{b_:.5f})'[{nxt}]")
        cur = nxt
    chain.append(f'[{cur}][1:v]overlay=0:0:eof_action=pass[v]')
    sh([FF, '-nostdin', '-v', 'error', '-y', '-i', 'cut/picture.mp4', '-i', 'cut/captions.mov',
        '-i', 'cut/his_mix.wav', *ins,
        '-filter_complex', ';'.join(chain), '-map', '[v]', '-map', '2:a',
    ...
    got = nframes('ad1_vertical_59s.mp4')
    assert got == total, f'cutdown is {got} frames, plan is {total}'
    print(f'ad1_vertical_59s.mp4 done -- {got} frames, {got/FPS:.2f}s')
