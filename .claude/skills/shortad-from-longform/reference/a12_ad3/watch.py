...
V  = sys.argv[1] if len(sys.argv) > 1 else 'ad3_vertical_9x16.mp4'
OUT = 'watch'
FPS = 30000/1001
...
def boundaries():
    ...
    # every overlay SUB-ELEMENT's in-time too (skill A6.19a): bullet items, headers, the COMPLETELY FREE tail, the note's
    # strike-throughs, the app's result screen, the punch-in ramps and his light-leak flashes
    for x in tl:
        for n in x.get('item_n', []): b.add(round(n/FPS, 2))
        if 't_hdr' in x: b.add(round(x['t_hdr']/FPS, 2))
        if x.get('tail'): b.add(round(x['tail'][1]/FPS, 2))
        if 'strikes' in x:
            for st in x['strikes']: b.add(round(x['t0'] + st, 2))
        if 'result_n' in x: b.add(round(x['result_n']/FPS, 2))
        for bl in x.get('blocks', []):                      # Ad 3: every block swap and every element's blur-in onset
            b.add(round(bl['n']/FPS, 2))
            for it in bl['items']: b.add(round(it['n']/FPS, 2))
        for s_, _p in x.get('stills', []): b.add(round(s_/FPS, 2))     # Ad 3: each still of his rapid photo sequence
    for n in BT.FLASHES: b.add(round(n/FPS, 2))
    for s in json.load(open('edl_picture.json')): b.add(round(s['n0']/FPS, 2))    # his picture cuts = our zoom cuts
    for h in json.load(open('crop.json'))['holds']:
        if h.get('ramp'): b.add(round(h['n0']/FPS, 2))                             # punch-in ramps
...
def scan(video):
    ...
    if len(glob.glob(f'{OUT}/all/*.png')) < BT.NTOT:
    ...
    known = set()
    for t in boundaries(): known.update(range(int((t-0.65)*FPS), int((t+0.65)*FPS)+1))  # a plate's hole opens up to ~0.5s after the beat t0
    for n in BT.FLASHES: known.update(range(n-2, n+3))
    for sgm in json.load(open('edl_picture.json')):        # HIS OWN cuts inside a talk beat (our zoom cuts)
        known.update(range(sgm['n0']-3, sgm['n0']+4))
...
if __name__ == '__main__':
    ...
    json.dump(dict(video=os.path.basename(V), boundaries=len(ts), reviewed=0,
                   inspected=False, scan={k: (v if not isinstance(v, list) else v[:40])
