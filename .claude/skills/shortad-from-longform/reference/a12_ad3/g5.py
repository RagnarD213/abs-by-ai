...
# His lower-third reveal, read frame by frame off his Ad 3 render (bar at 32.1 s, "AI has made personal trainers /
# like me totally obsolete."): the bar is up ~3 frames before the first letters, line 1 types over 6 frames (+1..+6),
# line 2 starts as line 1 lands and finishes at +12 -- the whole thing is done in 11 frames = 0.37 s. The inherited
# Ad 5 numbers (lead 0.20, span 0.80, stagger 0.55) took 46 frames = 1.55 s, 3.4x his, and opened on 6 frames of
# empty plate. Measured 2026-09-11 on all 8 of this ad's bars; see [A9] in SKILL.md.
LT_LEAD, LT_SPAN, LT_STAGGER, LT_GROW = 0.10, 0.21, 0.20, 0.10
...
def lower_third(lines, t, dur, y_bottom=1600, weights=None, sizes=None):
    ...
    o = 1.0 - ease_in_out(clamp01((t-(dur-0.12))/0.12)); gw = ease_out_cubic(clamp01(t/LT_GROW))  # his bars hold to the end; a 3-frame out
    ...
    for n, (s, f, lh) in enumerate(zip(lines, fs, lhs)):
        k = clamp01((t - LT_LEAD - n*LT_STAGGER)/LT_SPAN)
...
# His light leak is NOT white. Measured on four of his flashes (peaks 876, 1300, 4289, 6777): the whiteness he adds
# per channel is R 0.918-0.926, G 0.954-0.957, B 0.977-0.982 -- consistently blue-cyan, and his peak frames read
# RGB (240, 246, 251) where ours read neutral (audit 2026-09-11, item 16). Normalised to blue:
FLASH_TINT = (0.935, 0.971, 1.000)


def flash(frame_rgb, k, tint=FLASH_TINT):
    """A screen blend toward HIS light-leak colour (k = 1 is his full leak). Applied per frame from his envelope."""
    import numpy as np
    a = np.asarray(frame_rgb, dtype=np.float32)/255.0
    kc = np.asarray(tint, dtype=np.float32).reshape(1, 1, 3)*float(k)
    return ((1 - (1-a)*(1-kc))*255).clip(0, 255).astype("uint8")
