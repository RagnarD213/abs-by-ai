...
# mis-transcription burned into the captions is a spelling mistake in the ad.
FIX = {
    ('sixpackabs', '.com,'): ('SixPackAbs.com,', ''),     # Whisper splits it into two tokens
    ('sixpackabs.',): ('six-pack abs.',),                 # "...and get six-pack abs." (both Whispers glue it)
    ('growth', 'science'): ('bro-science', ''),           # medium: "gross science"; the script and his graphic: bro-science
    ('six', 'pack', 'ads'): ('six-pack', 'abs', ''),      # medium: "six -pack abs"
    ('with', 'ads,'): ('with', 'abs,'),                   # medium: "with abs,"
    ('I', 'use', 'AI'): ('I', 'used', 'AI'),              # medium: "I used AI"
    ('that', 'I', 'use', 'to'): ('that', 'I', 'used', 'to'),   # medium: "that I used to"
}
...
def suppressed():
    ...
    # ⚠ AD 3 USES ITS OWN `beats.muted_ranges()`, NOT the skill's inline list (2026-09-12).
    # They are NOT equivalent on this ad and swapping them silently changes the picture: Ad 3 also
    # mutes WINDOW beats whose body is in NO_CAPS_BODIES (his bullet screens print their own words),
    # and its CTA overlay kind is 'pill', not 'cta' -- the inline version would have run the captions
    # straight through every bullet screen AND every pill. Same intent, this ad's kind names.
