...
# ⚠ REPO is four levels up from HERE, which is right when this file sits in the skill's reference/ and WRONG in a
# build dir on the Extreme drive -- /Volumes/Extreme/_edit_work/ad3-vert resolves to "/", so FF became
# "/Media/video_edit/bin/ffmpeg" and qc.py died on FileNotFoundError AFTER every other gate had already passed.
# Fall back to PATH (the gate scripts export the real bin dir) rather than to a path that does not exist.
if not os.path.exists(FF):
    import shutil as _sh
    FF = _sh.which("ffmpeg") or FF
FP = FF.replace("ffmpeg", "ffprobe")
if not os.path.exists(FP):
    import shutil as _sh2
    FP = _sh2.which("ffprobe") or FP
SHARED = os.path.join(REPO, ".claude/skills/_shared/audio")
# ⚠ Same broken REPO as FF/FP above: in a build dir on the Extreme drive this resolved to "/…/_shared/audio"
# under "/", so row 18 reported "NO VALID STAMP: No module named 'require_stamp'" on a file whose stamp was
# present and PASSING. A gate that cannot find its own module must say so, not imply the file failed.
if not os.path.isdir(SHARED):
    _alt = "/Users/danielrose/Documents/Claude/Projects/Abs By AI/.claude/skills/_shared/audio"
    if os.path.isdir(_alt): SHARED = _alt
