...
OUT = sys.argv[2] if len(sys.argv) > 2 else "ad3_vertical_9x16.mp4"
...
# Write beside the deliverable and RENAME into place. A re-mux otherwise rewrites the file in place, and anything
# reading the deliverable at that moment -- an audit, a watch scan, a review copy -- gets a torn decode and reports
# a defect that does not exist. os.replace is atomic on one filesystem: an open reader keeps the old inode.
TMP = OUT + '.muxing.mp4'
r = subprocess.run([FF, '-nostdin', '-v', 'error', '-y', '-i', PIC, '-i', AUD, '-map', '0:v:0', '-map', '1:a:0',
                    '-c:v', 'copy'] + acodec + ['-movflags', '+faststart', TMP], capture_output=True, text=True)
if r.returncode:
