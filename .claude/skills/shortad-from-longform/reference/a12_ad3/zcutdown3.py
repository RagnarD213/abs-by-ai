...
Ad 3's cutdown (2026-09-12): hook (trainers are obsolete), credibility (a trainer who still got fat), proof (the 200 lb
picture -> the AI goal image -> the plan -> today), the generation for the viewer, what he gets after he taps, CTA.
Skill [R1]: the transcript is printed and read as prose FIRST; every seam is a sentence AND a thought boundary.
...
SHORT = '--short' in sys.argv          # the variant without "I knew exactly what to do..." (only if the full one runs over)
RANGES = [
 (0.0,                                            end('completely obsolete')),              # hook
 (at('listen ive spent thousands'),               end('a personal trainer myself')),        # he is a trainer himself
] + ([] if SHORT else [
 (at('i knew exactly what to do'),                end('still let myself go')),              # ...and still let himself go
]) + [
 (at('this is what i looked like'),               end('unhealthy dad bod')),                # proof: the 200 lb picture
 (at('but then i used ai'),                       end('look like today')),                  # the goal image -> the plan -> today
 (at('first you upload a picture'),               end('the motivation comes from')),        # the generation, for the viewer
 (at('your ai trainer is with you'),              end('every single exercise')),            # what he gets after he taps
 (at('so generate your future self'),             B.DUR),                                   # CTA (the AI-trainer beat) + tap the button
]
# ⚠ The first draft said the CTA TWICE -- "tap the button below to generate ... stubborn stomach fat" at 4886 AND
# "so generate your future self ..." at 7687. Both are Dan talking to camera, so the two of them spent 19.0 s of a
# 57 s ad (33 %) on a talking head repeating one instruction, and the cutdown measured 44.5 % insert coverage against
# the 55 % bound and HIS OWN 62.2 %. The fix is the selection, never the bound: the first CTA is dropped, the
# generation walkthrough runs on through the motivation phonecard, and the AI-trainer-at-every-workout beat takes its
# place -- so the ad now SHOWS the product where it used to talk about tapping. 63.0 % coverage, one CTA, at the end.
merged = []
for a, b in RANGES:
