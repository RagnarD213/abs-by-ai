...
def scan(video):
    ...
    if len(glob.glob(f'{OUT}/all/*.png')) < 6000:
        subprocess.run([FF,'-v','error','-y','-i',video,'-vf','fps=30000/1001,scale=96:96',
    ...
    for sgm in json.load(open('edl_picture.json')):        # HIS OWN cuts inside a talk beat
        known.update(range(int((sgm['cut_in']-0.15)*FPS), int((sgm['cut_in']+0.15)*FPS)+1))
    ...
    for t in ts:
        ...
        if not os.path.exists(c):
            subprocess.run([FF,'-v','error','-y','-ss',f'{max(0,t-1.0):.3f}','-t','2.0','-i',video,
                            '-vf','scale=270:270','-an','-c:v','libx264','-preset','veryfast',
