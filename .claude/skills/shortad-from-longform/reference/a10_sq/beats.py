#!/usr/bin/env python3
"""Ad 2 SQUARE 1:1 -- the APPROVED vertical's beat sheet, re-laid out. `beats_9x16_orig.py`
beside this file is that sheet, untouched, for diffing.

The timeline is identical to the frame: same EDL, same cuts, same pushes, same flashes, same
overlay times, same 8,275 frames. FOUR things differ, and all four are forced by the format or
by a standing rule that post-dates the vertical's approval:

  1. `before_dan` and `goal_dan` move from `bleed` to `card`. A 1:1 crop of a 3:4 photo takes a
     quarter of its height, and on both of these it takes his HEAD (proof:
     sqtest/_sheet_stills.png). In his card they are shown whole -- which is also his own
     language for media that does not fill his frame.
  2. The three real after pictures carry "Real picture of me -- not AI-generated"
     (AGENTS.md, Dan 2026-09-11: viewers were taking his real photos for AI). That rule is
     three days younger than the vertical's approval, so the vertical does not have it.
  3. The museum clip's chip and lower third come up the frame. MEASURED on the delivered square,
     not estimated: the exhibit's engraved plaque sits at y 800-925 (its text 830-895), because a
     1:1 crop of his 16:9 enlarges the whole case. The vertical's proportional placement (LT bottom
     at 80 % of frame height = 860) put the bar straight over "HUMAN NUTRITIONIST" and left only
     "1985-2025" showing -- the watch pass caught it, no metric could. LT bottom 795 (bar 642-795)
     clears the plaque; the chip moves to 520-594 to clear the LT.
  4. Nothing else. The conveyor's window offset moved in assets.py, for its own reason.

--- the vertical's own note, which still describes the sheet ---
...
# AGENTS.md, Dan 2026-09-11. The two labels are mutually exclusive and only AFTER pictures
# carry this one: the before photos (before_dan, fatdad_a, fatdad_b) are real and unlabelled,
# because nobody mistakes a before picture for an AI render -- that was never the complaint.
...
T = [
 ('talk',   0.000,   F(107), {}),
 ('bleed',  F(107),  F(216), dict(media='museum', label='AI-GENERATED', chip_y=520)),   # V2: hard cut in AND out; chip above his caption band, plaque left readable below
 ...
 ('talk',  F(4657), 157.250, {}),
 # ⚠ CARD, NOT BLEED, AT 1:1. before_dan is 675x900 and goal_dan 864x1184; a square crop of
 # either takes 25 % of the height and it comes off his head (sqtest/_sheet_stills.png shows
 # goal_dan reduced to a headless torso). Carded they are whole, the push is unchanged, and
 # Dan's 09-08 note -- "stomach visible" on the before photo -- is satisfied by construction.
 ('card', 157.250, 161.061, dict(media='before_dan')),
 ('card', 161.061, 165.230, dict(media='goal_dan', label='AI-GENERATED')),
 ('talk', 165.230, 169.903, {}),
 # Dan 2026-09-08: three different after pictures instead of one, each long enough to read.
 # Dan 2026-09-11 (AGENTS.md "Label Dan's real pictures"): every REAL after picture of him
 # carries the real-picture label for its full duration. All three survive a 1:1 crop with
 # head and waist in frame, so all three stay full-bleed -- "vertical and full screen".
 ('bleed',169.903, 170.703, dict(media='flag', label=REAL_LABEL)),
 ('bleed',170.703, 171.503, dict(media='after_standing', label=REAL_LABEL)),
 ('bleed',171.503, 172.303, dict(media='after_smiling', label=REAL_LABEL)),
 ('talk', 172.303, 183.170, {}),
...
OVERLAYS = [
 ...
 dict(kind='lt',  t0=F(107)+0.10, t1=F(216)-0.15, lines=['AI has made human nutritionists',
                                                        'completely obsolete.'], y_bottom=795),
 dict(kind='lt',  t0=F(2066), t1=73.250, lines=['2   Human nutritionists hand everybody',
                                               'the same generic meal plan.']),
 dict(kind='cta', t0=172.303, t1=176.000, top='Get A FREE AI Image of Yourself', big='With Abs'),   # starts after the three after pictures
