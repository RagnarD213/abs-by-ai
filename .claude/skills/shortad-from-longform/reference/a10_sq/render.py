#!/usr/bin/env python3
"""Render the 1:1 SQUARE picture: one output segment per beat, concat, then the overlays.

Re-laid out from the APPROVED 9:16 vertical; `render_9x16_orig.py` beside this file is that
render, untouched, for diffing. The EDL, the grade, the face track, the beat sheet, the push
schedule, the flashes and the caption alignment are all the vertical's, unchanged. What
changes here is geometry and nothing else:

  * VW, VH = 1080, 1080 and CROP_W = 1080. The talking head is a PURE CROP of the 1920x1080
    conform -- magnification 1.00 against the vertical's 1.78 -- so the unsharp the vertical
    needs to carry its upscale is REMOVED. Muhammad does not sharpen his own 1:1 pixels either.
  * The face track is stored as the LEFT EDGE of a 608 px window, so every x is shifted by
    (608-1080)/2 = -236 to become the left edge of a 1080 px window. Same centre, same track,
    same smoothing, same steps at his picture cuts.
  * Two stills move from full-bleed to his card: a 1:1 crop of a 3:4 photo of a person takes
    a quarter of its height, and on `before_dan` and `goal_dan` what it takes is his head.
...
BASE = 'base.mp4'
CROP_W = 1080
TRK_DX = (608 - CROP_W)//2      # the track is the 608 px window's left edge; ours is 1080 px wide
...
# ⚠ SHIFT THE TRACK, DO NOT RE-DERIVE IT. facetrack.json's x is the LEFT EDGE of the vertical's
# 608 px crop window; the same centre in a 1080 px window is that x - 236. Re-measuring the
# anchor would re-open [A4].1 (the colour heuristic), [A4].0ac (per-segment smoothing) and
# [A5].18 (frame-indexed sampling) for nothing -- the track is a property of the SOURCE, not of
# the output aspect. Range check: 522..768 becomes 286..532, nowhere near the 0..840 clamp.
_TRK['x'] = [float(v) + TRK_DX for v in _TRK['x']]
...
def talk_chain(t0, t1):
    """⚠ NO UNSHARP. The vertical's `unsharp=5:5:0.85` exists to carry a 1.78x upscale of a
    608 px crop. Here the crop is 1080 px and the output is 1080 px -- these are the conform's
    own pixels at 1.00x, which is exactly what Muhammad's 16:9 shows, and he does not sharpen
    them. Sharpening pixels that were never resampled only adds halos to his grade.
    (The push still reaches 1.20x for ~29 % of the talk; at that magnification a 1080 px source
    window is still 900 real pixels into 1080, far below where sharpening earns its artifacts.)

    The y expression is unchanged from the vertical and it means the same thing there and here:
    `ih` is the crop height, 1080 in both, so his 85 px upward recentre stays 7.9 % of the frame
    height in both formats."""
    z = push_z_expr(t0, t1)
    base = f"crop={CROP_W}:1080:'{crop_x_expr(t0, t1)}':0"
    if z is None:
        return f'{base},scale={VW}:{VH}:flags=lanczos'
    return (f"{base},zoompan=z='{z}':x='(iw-iw/zoom)/2':"
            f"y='(ih-ih/zoom)/2-{PUSH_UP:.1f}*(zoom-1)/{beats.PUSH_Z-1:.3f}':"
            f"d=1:s={VW}x{VH}:fps=30000/1001")
...
def media_wh(key):
    ...
    _WH[key] = tuple(int(x) for x in o.split('x')[:2])
    return _WH[key]

def bleed_sharpen(key, w=None, h=None):
    """⚠ SHARPEN ONLY WHAT WAS RESAMPLED. In 9:16 every full-bleed clip is a cover-crop plus a
    scale, so the vertical sharpens all of them. At 1:1 a 1920x1080 clip is cover-cropped to
    1080x1080 with NO scale at all -- the conform's own pixels -- and an unsharp on unresampled
    pixels is a halo, not a recovery. So the filter is attached only where the cover actually
    changes scale by more than 2 %."""
    w = w or VW; h = h or VH
    sw, sh = media_wh(key)
    s_ = max(w/sw, h/sh)
    return ',unsharp=5:5:0.4:5:5:0.0' if s_ < 0.98 or s_ > 1.02 else ''
...
def vlib_chip(label, path, y=784):
    ...
    face: it sits at the shorts/waistline, clear of the caption band (CAP_Y 880), sized to
    actually read. `y` overrides the height per beat.

    ⚠ THE REAL-PICTURE STRING IS THREE TIMES THE LENGTH OF "AI-GENERATED" and at the chip's
    52 px it is 1245 px wide in a 1080 px frame. The size steps down until the box fits inside
    the SAME 100 px margins the type uses -- it lands at 44 px, exactly the fallback size the
    Ad 5 handoff specifies, in an 875 px box at x 102..977, so the label stays clear of the
    in-feed reserve on the right. It never wraps: a two-line chip at the waistline starts
    covering the abs the picture exists to show. "AI-GENERATED" is 439 px and never shrinks."""
    f = font(52, 'ExtraBold')
    while f.size > 30 and text_size(label, f)[0] + 56 > VW - 2*vlib.MARGIN:
        f = font(f.size - 2, 'ExtraBold')
...
def _sig(b, nfr, t0):
    v = 'sq1'      # SQUARE geometry: never share a segment cache with the 9:16 build; the media spec and the track slice are hashed separately
...
def render_bleed_frames(key, n, out, amt=0.075):
    ...
    ch = still_chain(VW, VH, n, amt=amt, **o) if isimg else \
         media_prefix(key) + cover_chain(VW, VH, **o) + bleed_sharpen(key)
...
def render_segment(i, b, nfr, t0):
    ...
    if k == 'bleed':
        ...
        bchain = (still_chain(VW, VH, nfr, amt=amt, **o) if isimg
                  else media_prefix(b['media']) + cover_chain(VW, VH, **o) + bleed_sharpen(b['media']))
        lab = b.get('label')
        if lab:
            cy = int(b.get('chip_y', 784))
            # ⚠ the fitted SIZE is part of what was drawn, so it is part of the key -- a key of
            # label+y alone served a 50 px chip after the fit limit changed (the cache-key class
            # this skill has now paid for four times: index, media spec, media aspect, and this)
            chip = f'gfx/chip_{hashlib.md5(f"{lab}@{cy}@{VW}@{vlib.MARGIN}".encode()).hexdigest()[:8]}.png'
    ...
    if 'dan' in holes:
        ...
        cw, ch, cx, cy = vlib.window_crop(h, w)
...
def main():
    ...
    for n, (kind, a, b, spec) in enumerate(items):
        h = hashlib.sha1(json.dumps([kind, spec, round(b-a, 4), 'sq1'], sort_keys=True,
