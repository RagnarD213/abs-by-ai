...
for r, f in zip(R, fs):
    ...
    if wall_lvl is None or wall_lvl - hair_lvl < 25:
        r['hair'] = ym; r['hair_src'] = 'mask'           # hair touches the top or no contrast: trust the mask
    else:
        thr = (wall_lvl + hair_lvl) / 2
        y = next((y for y in range(0, min(1079, ym + 30)) if prof[y] < thr and prof[y+1] < thr and prof[y+2] < thr), ym)
        r['hair'] = int(y); r['hair_src'] = 'luma'; r['wall'] = round(wall_lvl); r['hlvl'] = round(hair_lvl)
    out.append(r)
