...
V  = sys.argv[1] if len(sys.argv) > 1 else 'ad2_vertical_9x16.mp4'
OUT = 'watch'
FPS = 24.0
...
def boundaries():
    ...
    # every overlay SUB-ELEMENT's in-time too: a subtitle cached out of its lower third (2026-09-10) fell between the
    # boundaries and passed the watch pass, so subtitles, checklist items and an end card's second line are boundaries
    for x in ov:
        if 'sub' in x: b.add(round(x['sub']/FPS, 2))
        for n in x.get('items', []): b.add(round(n/FPS, 2))
    for x in tl:
        if 't_second' in x: b.add(round(x['t0'] + x['t_second'], 2))
    ...
    for n in (2615, 3875): b.add(round(n/FPS, 2))                                  # his punch-in ramps
...
def scan(video):
    ...
    if len(glob.glob(f'{OUT}/all/*.png')) < BT.NTOT:
        subprocess.run([FF,'-v','error','-y','-i',video,'-vf','fps=24,scale=96:171',
