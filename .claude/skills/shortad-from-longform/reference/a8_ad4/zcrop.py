...
RAMP = 6
FORCE_VISIBLE = {2937, 3543, 5905}      # the Ad 4 audit (2026-09-11) read these as naked jump cuts at the phone's
                                        # magnification: 2937 a blink-jump (head ~80 px, ratio 1.11), 3543 hands up -> down
                                        # under point 4 (ratio 1.6). A level change hides both (skill A7.15).
                                        # 5905/5912 (audit 2): with W4 ending on his cut (5905) the level change stayed at the
                                        # OLD window edge (5912) -- 7 FAR frames then a punch on continuous footage. The change
                                        # now sits ON the window exit (5905 visible, 5912 invisible): same group count, so no
                                        # other hold's level moves
FORCE_INVISIBLE = {1267, 5912, 6508}    # 1267: his splice 3 frames after the W1 window exits (1264): as a visible join it made
                                        # a 3-frame NEAR sliver between the window and a FAR hold; the window's exit hides it.
                                        # 6508: his splice 6 frames before the supermarket card, inside the punch -- as a new
                                        # group it re-anchored y0 (+13 phone px) for 6 frames; the level and y0 now carry
