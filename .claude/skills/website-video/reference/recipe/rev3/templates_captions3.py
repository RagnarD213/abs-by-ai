...
VOUT = os.environ.get("VOUT", f"{HERE}/ad3_16x9.mp4")
TX   = f"{HERE}/final.whisper.json"

# Full-screen cards already carry a headline; a caption on top is two competing texts.
SUPPRESS = [B.WHYCARD, B.BROCARD, B.HOWCARD, B.BEFORE, B.GOALIMG, B.TODAY,
            B.CTA1, B.CTA2,
            # app/insert panels: their UI text is dense and it IS the product demo
            B.APPGEN, B.ASSESS, B.WORKOUT, B.DEMOS, B.ROBOTCUT, B.ROBOTSTORY, B.SPACLIP]
PANELS   = [B.COSTCARD, B.TAILOR, B.PLANBUL, B.ADAPTS, B.ADAPTMID]
LOWERS   = [B.LOWER3A, B.NUM1, B.NUM3, B.NUM4, B.TOOLATE, B.GYMQ, B.NOTPRICE]
...
CORRECT = {r"\bSixpackabs\.com\b": "SixPackAbs.com",
           r"\bSixpackabs\b": "six-pack abs",
           r"gross[- ]science": "bro-science", r"grow[- ]science": "bro-science",
           r"\bsix pack\b": "six-pack", r"lacking body parts": "lagging body parts"}
...
def build():
    ...
    for w in ws:
        if cur and (w["start"] - cur[-1]["end"] > 0.55 or len(cur) >= 4): flush()
        cur.append(w)
        t = w["word"].strip()
