...
VW, VH = 1080, 1920     # VERTICAL (AV-01: the approved 9:16 geometry)
CAP_Y  = vlib.CAP_Y
F      = font(64, "ExtraBold")
MAXW   = VW - 150
...
SCRIM = 0.00          # peak opacity of the soft dark bed behind the caption band
SCRIM_Y = (820, 885, 995, 1055)   # ramp up, hold, ramp down -- a soft bed, not a bar

_SCRIM_CACHE = {}
def scrim_layer():
    """⚠ A GREEN HIGHLIGHT OVER GREEN-TONED FOOTAGE IS UNREADABLE -- TO THE EYE AND TO THE
    SYNC GATE -- AND THE FIX IS THE PICTURE, NOT THE GATE (skill lesson A6.17).

    On this cut the hook's caption band lands on the NEON YELLOW-GREEN stripe of his
    shorts: measured 16,500-18,400 pixels within 90 levels of the olive highlight in the
    band, and the delivered file lit 'me', 'abs' and 'it's' invisibly over it (three
    consecutive words -- `caption_sync_check.py` failed the build on the run, at 98.4 %
    overall). The tempting fix, teaching the gate this build's exact highlight colour, was
    ruled out the same way A6.17 ruled it out: no tolerance separates the word from that
    background, because the EYE cannot separate them either.

    So a soft dark bed goes behind the band, under the text, on every caption state. It
    puts the background under both thresholds while the lit word stays full colour."""
    if 'a' in _SCRIM_CACHE: return _SCRIM_CACHE['a'].copy()
    import numpy as np
    y0, y1, y2, y3 = SCRIM_Y
    a = np.zeros(VH, dtype=np.float32)
    t = np.clip((np.arange(y0, y1) - y0) / (y1 - y0), 0, 1)
    a[y0:y1] = t*t*(3 - 2*t)
    a[y1:y2] = 1.0
    t = np.clip((np.arange(y2, y3) - y2) / (y3 - y2), 0, 1)
    a[y2:y3] = 1 - t*t*(3 - 2*t)
    lay = Image.fromarray(np.zeros((VH, VW, 4), dtype=np.uint8), 'RGBA')
    arr = np.zeros((VH, VW, 4), dtype=np.uint8)
    arr[..., 3] = (a[:, None] * SCRIM * 255).astype(np.uint8)
...
def render(gs, out='captions.mov', capdir='cap', stops=None):
    ...
    # the STOPS must be the stops of the FILE BEING RENDERED. Recomputed from the beat sheet
    # they are the MASTER's times -- meaningless in a cutdown, whose timeline has had time
    # removed from under it. The delivered cutdown printed a held "below to" straight across
    # the CTA pill because the hold was clipped at a master timecode absent from the cut.
    STOPS = (sorted(set(stops)) if stops is not None else
             sorted(set([a_ for a_, b_ in suppressed()] + list(getattr(BT, 'SEAMS', [])))))
    ...
    for g in gs:
        ...
        for k, (w, ws, we) in enumerate(g):
            ...
            sig = hashlib.md5(f'{txt}|{k}|{CAP_Y}|{SCRIM}|{SCRIM_Y}'.encode()).hexdigest()[:10]
            p = f"{capdir}/c{n:05d}_{sig}.png"; n += 1
