...
def window_crop(win_h, cx=None):
    ...
    the 1.78x upscale the full-bleed mode is forced into. `cx` (attempt-3 rev 1) is the
    beat's measured face centre -- Dan leans, so the fixed SUBJECT_CX left him off-centre
    in some windows."""
    if cx is None:
...
def overlay_inset_photo(img_path, dur, fps=FPS, rect=(30, 545, 270, 900)):
    """ATTEMPT 3 rev 4: the hook's AFTER picture as an inset graphic over the talking
    head -- his 0:00 framed the physical print with an animated callout; the print is
    outside our 9:16 crop, so per Dan's fallback the picture itself goes on screen,
    popped in with his ease-out-back settle and wearing his olive callout stroke."""
    x0, y0, x1, y1 = rect
    w, h = x1-x0, y1-y0
    ...
    for i in range(nframes(dur, fps)):
        ...
        ImageDraw.Draw(sh).rounded_rectangle([bx, by, bx+ww, by+hh], radius=18,
                                             fill=(0,0,0,int(160*o)))
        ...
        ImageDraw.Draw(m).rounded_rectangle([0, 0, ww, hh], radius=18, fill=a)
        ph.putalpha(m)
        im.alpha_composite(ph, (bx, by))
        # his callout stroke is a thin LIGHT CYAN-WHITE box (measured off his 0:00
        # frames), not the olive of the plates
        p = ease_out_cubic(clamp01((t-0.30)/0.45))
        if p > 0:
            hw2, hh2 = ww/2*(1.10-0.10*p)+9, hh/2*(1.10-0.10*p)+9
            ImageDraw.Draw(im).rounded_rectangle(
                [cx-hw2, cy-hh2, cx+hw2, cy+hh2], radius=14,
                outline=(214, 236, 244, int(255*o*p)), width=5)
        out.append(im)
    return out, None
