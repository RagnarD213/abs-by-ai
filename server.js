...
app.post('/api/generate-image', aiLimiter, (req, res, next) => optionalAuth(req, res, next), async (req, res) => {
  try {
    ...
    const buildVerifierQuestion = (who, intens, cond) => {
      ...
      // MALE strong: every clause is a COMPARISON against the before photo. The old
      // wording asked "is there visible ab definition?" — a presence check, which any
      // man who already had abs passes from the before photo alone. That let a total
      // no-op sail through for exactly the lean/fit users who most need the retry.
      return strong
        ? 'Compare these two photos of the same person. Ignore tan, lighting, camera quality, and background — judge only the body. The question is NOT whether the AFTER looks good; it is whether the AFTER shows a CLEAR INCREASE over the BEFORE. If the subject ALREADY had visible abs or an athletic build in the BEFORE photo, that does NOT count on its own — you must see a clear increase BEYOND what was already there, either in muscle size (fuller/thicker chest, larger and rounder shoulders, wider lats, noticeably thicker arms) or in leanness and definition (more ab separation, a tighter and more tapered waist). Answer NO if the AFTER could plausibly be mistaken for the BEFORE, or if you had to look closely to find the difference. Answer YES only if an ordinary viewer would immediately see that the AFTER body is more muscular and/or leaner. Reply with only YES or NO.'
    ...
    if (result.ok && rungBudget > 0 && !chooserCandidates) {
      ...
      // Male preambles are condition-aware. Only men who START lean/fit get the
      // contest-lean six-pack push; heavier/moderate men get a whole-body "major fat
      // loss + visibly more muscle" push — A3.1 proved Gemini refuses a shredded-peak
      // ask on heavier bodies, so those retries burned Gemini calls and changed nothing.
      const maleLeanPreambles = [
        `Your previous attempt at this edit was too subtle and barely visible — that is a failure. This time you MUST push much harder on BOTH axes: make the subject visibly BIGGER — a fuller, thicker chest, distinctly larger and rounder shoulder caps, wider lats, noticeably thicker arms — AND strip the torso to a genuinely shredded condition with a fully visible, sharply cut SIX-PACK: all six blocks separated by crisp shadow lines, a sharp vertical midline, defined obliques, a visible V-cut into the waistband, and NO softness anywhere on the torso. Even if the subject's torso is angled, partly turned, or partly covered by an arm in this photo, the visible portion MUST still be fully transformed. The face, clothing, pose, and framing must stay exactly the same — only push the body transformation much further.\n\n`,
        `Two previous attempts at this edit were both too subtle. This is the FINAL attempt and the body MUST look unmistakably bigger and more developed than the input — a visibly thicker chest, clearly larger and wider shoulders, wider lats, noticeably thicker arms — with a dramatically shredded torso: a deeply cut six-pack with crisp separation between every abdominal block, a sharp midline, defined obliques, visible serratus, a clean V-cut into the waistband, and zero softness left. Redraw the torso as much as it takes: a partly angled, turned, or arm-obscured torso is NOT a reason to leave the body unchanged. Do not return an image that could be mistaken for the input. The face, clothing, pose, and framing must stay exactly the same — only the body transformation gets much more dramatic.\n\n`,
      ];
      const maleHeavierPreambles = [
        `Your previous attempt at this edit was too subtle and barely visible — that is a failure. This time the change MUST be obvious at a glance: a MAJOR, whole-body reduction in body fat — a visibly flatter, tighter midsection and waist with clear ab definition, no soft belly or love handles — PLUS visibly more muscle underneath: a fuller chest, larger and rounder shoulders, wider lats, and thicker arms. The result must look like someone who lost serious fat AND built real muscle, never a slightly slimmer copy of the input. The face, clothing, pose, and framing must stay exactly the same — only push the body transformation much further.\n\n`,
        `Two previous attempts at this edit were both too subtle. This is the FINAL attempt and the transformation MUST be unmistakable: strip away a large amount of body fat across the entire torso — a clearly flatter stomach, a much tighter waist, visible ab muscle definition with separation lines — and make the physique visibly more muscular, with a fuller chest, broader shoulders, and a clear V-taper. Redraw the body as much as it takes; do not return an image that could be mistaken for the input. The face, clothing, pose, and framing must stay exactly the same — only the body transformation gets much more dramatic.\n\n`,
      ];
      const leanStartMale = (startCondition === 'fit' || startCondition === 'very_lean');
      const intensifyPreambles = sex === 'female' ? femalePreambles
        : leanStartMale ? maleLeanPreambles
        : maleHeavierPreambles;
      let changed = false;
